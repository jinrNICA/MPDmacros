...
const Long_t range_dca[]={1e5,2e8};
...
void Draw_TDR()
{
    ...
    TLegend* leg_pid = new TLegend(0.68,0.23,0.87,0.42);
    ...
    leg_res->AddEntry(p_res_laqgsm_vs_centrality[0],energy_title[i_energy].Data(),"l");
    ...
    leg_vn_pt[i_energy] = new TLegend(0.22,0.68,0.48,0.86);
        for (Int_t i=0;i<4;i++){
            leg_vn_type[i_energy][i] = new TLegend(0.66,0.24,0.85,0.42);
        }
    }
    ...
    leg_vn_rapidity[i_energy] = new TLegend(0.22,0.68,0.48,0.86);
        for (Int_t i=0;i<4;i++){
            leg_vn_type_rapidity[i_energy][i] = new TLegend(0.66,0.24,0.85,0.42);
        }
    }
